...
class Trader:
    POSITION_LIMIT = {'COCONUT': 300, 'COCONUT_COUPON': 600, 'AMETHYSTS':20, 'STARFRUIT': 20, 'ORCHIDS': 100, 'GIFT_BASKET': 60, 'STRAWBERRIES': 350, 'CHOCOLATE': 250, 'ROSES': 60}
    ourPosition = {'COCONUT':0, 'COCONUT_COUPON':0, 'AMETHYSTS':0, 'STARFRUIT': 0, 'ORCHIDS': 0, 'GIFT_BASKET': 0, 'STRAWBERRIES': 0, 'CHOCOLATE': 0, 'ROSES': 0}
    bestBids =  {'COCONUT':[0,0], 'COCONUT_COUPON':[0,0],'AMETHYSTS':[0,0], 'STARFRUIT': [0,0], 'ORCHIDS': [0,0], 'GIFT_BASKET': [0,0], 'STRAWBERRIES': [0,0], 'CHOCOLATE': [0,0], 'ROSES': [0,0]}
    bestAsks = {'COCONUT':[0,0], 'COCONUT_COUPON':[0,0],'AMETHYSTS':[0,0], 'STARFRUIT': [0,0], 'ORCHIDS': [0,0], 'GIFT_BASKET': [0,0], 'STRAWBERRIES': [0,0], 'CHOCOLATE': [0,0], 'ROSES': [0,0]}
    products = ['COCONUT', 'COCONUT_COUPON','AMETHYSTS', 'STARFRUIT', 'ORCHIDS', 'GIFT_BASKET', 'STRAWBERRIES', 'CHOCOLATE', 'ROSES']
    beta = 50 
    chocolatePrice =[]
    rosesPrice = []
    strawberriesPrice = []
    ...
    def computeCouponPrice(self, coconutPrice, timestamp):
        sigma = np.sqrt(((250*10000 - timestamp/100)*v))
        limit = int(10000+3*sigma)
        s = [
            (x - K)/(sigma*np.sqrt(2*np.pi))*np.exp(-(x-coconutPrice)**2/(2*sigma**2)) for x in range(K, limit)
        ]
        return sum(s)
    ...
    def run(self, state: TradingState):
        ...
        if state.timestamp >= self.beta*100:
            # pairs trade basket and choc
            chocAndBasketCorr = self.findCorrelation(self.chocolatePrice[-self.beta:], self.gift_basketPrice[-self.beta:])
            dataCorrelation = self.findCorrelation(self.NAV[-self.beta:],self.gift_basketPrice[-self.beta:])
        else: 
            chocAndBasketCorr = 0.6 # I do not wish to trigger pairs trading when the dataCorrelation is not low enough to indicate drift, nor high enough to indicate they are paired together again
            dataCorrelation = 0.6
        for product in state.order_depths: 
            ...
            if product == 'COCONUT_COUPON':
                self.coconutBidPrice = list(state.order_depths[product].buy_orders.items())[0][0]
                self.coconutAskPrice = list(state.order_depths[product].sell_orders.items())[0][0]
                midPrice = (self.coconutAskPrice + self.coconutBidPrice)/2
                fairCouponPrice = int(self.computeCouponPrice(midPrice, state.timestamp))
                # bid: bots buy
                # ask: bots sell
                if fairCouponPrice > self.bestAsks[product][0]:
                    orders.append(Order(product, self.bestBids[product][0], -self.bestBids[product][1]))
                elif fairCouponPrice < self.bestBids[product][0]:
                    orders.append(Order(product, self.bestAsks[product][0], -self.bestAsks[product][1]))
            if product == 'GIFT_BASKET':
                pass
                """if chocAndBasketCorr < 0.5:
                    if self.chocolatePrice[-1] > self.gift_basketPrice[-1]:
                        # buy basket, sell component
                        basket_buy_amount = self.b_multiplier*self.findBasketAmount(chocAndBasketCorr) # without any consideration of limits
                    ...
                    elif self.chocolatePrice[-1] < self.gift_basketPrice[-1]:
                        # sell basket, buy components
                        basket_sell_amount = self.findBasketAmount(chocAndBasketCorr)
            ...
            if product == 'CHOCOLATE':
                ...
                pass
                """basket_buy_amount = self.findBasketAmount(chocAndBasketCorr)
                if chocAndBasketCorr < 0.5:
                ...
                if chocAndBasketCorr > 0.8 and self.ourPosition['GIFT_BASKET'] >0:
...
# our code didn't trade coupons. PnL is -9400
# while roses and basket and choc and basket continue to have high correlation, str have -0.345 corr. Pairs is not good strat
# stopped trading roses. Everything else same, pnl -3321. So it's less negative. 
# strawberries are highly positive